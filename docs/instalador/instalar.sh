...
actualizarBDD()
{
   ...
   tail -n2 /tmp/$ID.permisosbdd2 >> /tmp/UPDATES_MERAN_SANDBOX.sql
   mysql -h$HOST_BDD_MERAN --default-character-set=utf8 -u$ROOT_USER_BASE --password=$ROOT_PASS_BASE < /tmp/UPDATES_MERAN_SANDBOX.sql
   if [ $? -ne 0 ]
  then
    echo 'No fue posible conecatarse a la base de datos con los datos suministrados actualizar'
    mkdir ~/pendiente
...
generarPermisosBDD()
{
  ...
  cat $sources_MERAN/../../../configuracion/mysql/meran.sql >>  /tmp/$ID.permisosbdd
